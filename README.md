...
- **Devices** _(optional — toggle in Settings → Devices)_ — manage client and internal devices/assets with configurable type and status, serial/manufacturer/model, hostname, purchase and warranty dates, and client assignment; list with KPIs (incl. warranty expiring soon) and a section on the client detail. RBAC-gated by `devices.*` permissions.
